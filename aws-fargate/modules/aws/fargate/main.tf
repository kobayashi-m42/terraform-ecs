...
resource "aws_ecs_task_definition" "api_fargate" {
  ...
  cpu                      = "${lookup(var.fargate, "${terraform.env}.task_cpu", var.fargate["default.task_cpu"])}"
  memory                   = "${lookup(var.fargate, "${terraform.env}.task_memory", var.fargate["default.task_memory"])}"
...
resource "aws_ecs_service" "api_fargate_service" {
  ...
  desired_count   = "${lookup(var.fargate, "${terraform.env}.service_desired_count", var.fargate["default.service_desired_count"])}"
  launch_type     = "FARGATE"
